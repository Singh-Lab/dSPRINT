...
if __name__ == "__main__":
	...
	for x in [os.path.join(nndir, '../dat'), "."]:
		if os.path.isfile(os.path.join(x, 'pp1.npz')):
				nndir = x; break
	else:
		print >>sys.stderr, "no lib file found:", os.path.join(nndir, 'pp1.npz')
		exit(1)
#	if os.path.isfile(os.path.join(nndir, '../dat/pp1.npz'): nndir += '../dat/'

	dict1_nn = load_NN(os.path.join(nndir,'pp1.npz'))
	dict2_nn = load_NN(os.path.join(nndir,'pp2.npz'))
	dict3_nn = load_NN(os.path.join(nndir,'pp3.npz'))
